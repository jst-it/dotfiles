...
Panel {
  ...
  readonly property var hiddenSinkNames: [
    "alsa_output.pci-0000_0d_00.6.analog-stereo",
    "alsa_output.usb-Sony_Interactive_Entertainment_DualSense_Edge_Wireless_Controller-00.Direct__Direct__sink"
  ]
  ...
  readonly property var candidateSinks: {
    ...
    for (var i = 0; i < nodes.length; i++) {
      var n = nodes[i]
      if (n && n.isSink && !n.isStream && hiddenSinkNames.indexOf(String(n.name || "")) < 0) list.push(n)
    }
    return list
  }

  readonly property var hiddenSourceNames: [
    "alsa_input.usb-Sony_Interactive_Entertainment_DualSense_Edge_Wireless_Controller-00.Direct__Direct__source",
  ...
  readonly property var candidateSources: {
    ...
    for (var i = 0; i < nodes.length; i++) {
      ...
      if (n && !n.isSink && !n.isStream && isAudioSource(n)) {
        ...
        if (hiddenSourceNames.indexOf(name) >= 0) continue
        list.push(n)
      }
    }
    return list
