...
-- Sections below mirror the groups the SUPER+K menu shows. That grouping is
-- applied by ~/.local/bin/omarchy-menu-keybindings-relabeled, which sorts on
-- the *description* text - so renaming a binding here can move it between
-- groups in the menu. The package-owned omarchy-menu-keybindings does not
-- read this file's layout at all; the sections are for humans.
...
-- SUPER+SHIFT+C is re-used further down for Claude Code.
...
-- (e.g. "KP_END" -> "NUM1") for display and groups the list by category,
-- without forking the package-owned omarchy-menu-keybindings. See
...
-- The SUPER+K menu shows these as NUM1..NUM0 rather than the raw KP_* keysyms.
